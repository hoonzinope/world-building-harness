...
func TestStoryLifecycleActionsAuditAndGuardActiveJobs(t *testing.T) {
	...
	if _, err := store.exportStoryBundle(busyID, &Actor{ID: "user_admin", Role: "admin"}); err == nil {
...
func TestStoryAdminTurnEditAndRollbackUpdateProjection(t *testing.T) {
	...
	if err := store.appendChoice(id, &Actor{ID: "user_admin", Role: "admin"}, "", "action", "베이르가 아닌 루세라에 있어야 한다고 주장한다"); err != nil {
		t.Fatal(err)
	}
