...
func GenerateLocalGMScene(prev Turn, st State, inputLabel, mode string) string {
	if inputLabel == "" {
		inputLabel = "직접 행동"
	}
	name := "인물"
...
func GenerateNextChoices(turn int, st State) []Choice {
	name := "인물"
	if len(st.ActiveCharacters) > 0 && strings.TrimSpace(st.ActiveCharacters[0]) != "" {
		name = strings.TrimSpace(st.ActiveCharacters[0])
	}
...
func SummarizeQuestionAnswer(question string, st State) string {
