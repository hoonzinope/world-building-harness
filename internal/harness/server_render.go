...
func (s *webServer) render(w http.ResponseWriter, r *http.Request, title, body string, data map[string]any) {
	...
	t, err := template.New("page").Funcs(template.FuncMap{
		...
		"packURL":                        func(base, pack string) string { return fmt.Sprintf("%s/packs/%s/", base, url.PathEscape(pack)) },
		"storyURL":                       func(base, id string) string { return fmt.Sprintf("%s/stories/%s", base, url.PathEscape(id)) },
		"sceneJournalTitle":              func(turnID int, title string) string { return storyTurnTitle(turnID, title, "세션 기록") },
		"sceneIndexTitle":                func(turnID int, title string) string { return storyTurnTitle(turnID, title, "") },
		"storyTurnTimestamp":             func(timestamp string) string { return storyTimestampKST(timestamp, "시각 확인 불가") },
		...
		"idem":                           func() string { return mustTurnIdempotencyKey() },
		"eq":                             func(a, b any) bool { return fmt.Sprint(a) == fmt.Sprint(b) },
		"not":                            func(v bool) bool { return !v },
		"nl2br":                          func(s string) template.HTML { return template.HTML(template.HTMLEscapeString(s)) },
