...
const storyRoomTemplateAdminPanels = `
      {{if .IsAdmin}}
      <details class="admin-drawer panel">
        <summary>
          <span>
            <strong>관리</strong>
            <span class="muted">운영 패널</span>
          </span>
        </summary>
        <div class="admin-panel-body">
          <section class="admin-cluster" aria-label="상태와 진행자">
            <h3>상태</h3>
          ...
          {{if .CanAdminMutate}}
            {{with .LatestTurn}}
            <section class="admin-cluster" aria-label="현재 턴 편집">
              <h3>현재 턴 {{$.LatestTurnID}} 편집</h3>
              <form method="post" action="{{$.Base}}/stories/{{$.Story.ID}}/admin">
                <input type="hidden" name="csrf_token" value="{{$.CSRFToken}}">
          ...
          {{end}}
          <section class="admin-cluster" aria-label="내보내기">
            <h3>내보내기</h3>
            <form method="post" action="{{.Base}}/stories/{{.Story.ID}}/admin">
              <input type="hidden" name="csrf_token" value="{{.CSRFToken}}">
          ...
          <section class="admin-cluster admin-danger-cluster" aria-label="위험 작업">
            <h3>위험 작업</h3>
            {{if .CanAdminMutate}}
            <form method="post" action="{{.Base}}/stories/{{.Story.ID}}/admin">
              <input type="hidden" name="csrf_token" value="{{.CSRFToken}}">
