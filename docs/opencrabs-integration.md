...
### path violation
`world-tool`은 world root 밖 경로 접근을 error로 반환한다. 예외는 두 가지뿐이다. 첫째, registry/config 경로는 `--registry`, `WORLD_TOOL_REGISTRY`, `~/.opencrabs/worlds.yaml`, `~/.config/world-tool/worlds.yaml`에 한해 read/write가 가능하지만, normalize와 symlink 검사를 통과한 configured registry/config 접근만 허용한다. 둘째, `approval attest`의 `auth_context_file`은 location boundary를 먼저 검증한 뒤 signature/MAC/trust-material 검증과 issuer/audience/scope/expiry 검증을 통과한 trusted auth context input에 한해 read-only로 읽을 수 있다. 이 두 예외 모두 선택된 world root 밖의 `content/`, staging, `runs/` 경로를 허용하지 않는다. production에서는 먼저 configured wrapper-owned auth-context directory의 normalized regular file 또는 trusted wrapper request-file/FD mechanism인지 location validation을 통과해야 하고, parent traversal, symlink, selected world root, runtime-owned run directory, staged inbox path를 거부한 뒤에만 wrapper-signed 또는 MACed envelope를 configured wrapper trust material으로 검증하며 expected issuer/audience/scope/expiry policy를 만족해야 한다. local fixture mode는 test-only이고 explicit opt-in이 필요하다. hash/expiry만으로는 충분하지 않다.
