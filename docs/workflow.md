...
- `force`는 `approval_attestation_file`, `approval_attestation_hash`, `approver_id`, `approval_channel`, `authenticated_actor`가 함께 기록될 때만 승인 provenance가 완성된다. staged approval attestation의 embedded downstream_action은 `world_force_accept_draft`와 exact match여야 한다. canonical 세부 계약은 `docs/commands.md`를 따른다.
