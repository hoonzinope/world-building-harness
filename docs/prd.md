...
4. `world init/status/list`, `registry add/list/remove/default`, `input stage`, `doc list/read/search`, `content validate`, `draft create/update/read/list/validate/diff/accept/reject`, `approval attest`, `run get`, `run get --artifact <basename>`, `run recover` 같은 MVP 핵심 명령 가족과 JSON 계약
