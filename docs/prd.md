...
### Accept
사용자가 `world_diff_draft` 결과의 diff summary와 binding 값을 확인한 뒤, OpenCrabs는 reason을 staging한다. 그 다음 사용자는 diff와 staged reason file/hash를 명시적으로 승인해야 하며, 그 승인 확인 후에만 OpenCrabs는 diff_run_id, draft_hash, target_base_hash, patch_hash, reason hash를 확인하고 `world_create_approval_attestation`으로 auth context와 diff/reason binding을 묶는다. 이 auth context는 world root 밖 trusted auth context input이어야 하고, production에서는 먼저 configured wrapper-owned auth-context directory의 normalized regular file 또는 trusted wrapper request-file/FD mechanism인지 location validation을 통과해야 하며, parent traversal, symlink, selected world root, runtime-owned run directory, staged inbox path를 거부한 뒤에만 wrapper-signed 또는 MACed envelope와 configured wrapper trust material, expected issuer/audience/scope/expiry policy를 만족해야 한다. `world_create_approval_attestation`은 `world_accept_draft` 또는 `world_force_accept_draft` 중 정확히 하나의 downstream action을 받아 staged approval attestation에 기록해야 하며, auth context input의 exact downstream action과 accept/force는 그 값과 exact match여야 한다. exact downstream action이 없거나 mismatch면 `AUTH_CONTEXT_SCOPE_DENIED`, staged attestation이 later accept/force와 mismatch면 `APPROVAL_ATTESTATION_BINDING_MISMATCH`다. 세부 계약은 `docs/commands.md`를 따른다. 그 다음 `world_accept_draft` tool을 호출한다. tool은 diff binding과 validation을 재검증하고, conflict나 policy stop이 없을 때만 `content/`로 승격한 뒤 accepted draft를 archive한다. operator-approved force path가 필요한 경우에도 approval attestation/provenance는 유지되어야 하며, `approval_channel`은 attest/accept/audit 전반에서 byte-identical이어야 한다.
