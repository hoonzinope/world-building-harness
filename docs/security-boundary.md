...
- symlink resolution
- `--auth-context-file`은 staging file이 아니며, `approval attest`에서만 사용한다. production file은 먼저 configured wrapper-owned auth-context directory 아래의 normalized regular file이거나 trusted wrapper request-file/FD mechanism인지 location validation을 통과해야 한다. parent traversal, symlink, selected world root 내부, runtime-owned run directory, staged inbox path는 hash/parse/signature/MAC/trust-material 검증 전에 거부하고, 그 다음 hash, parse, signature, MAC, trust-material 검증과 issuer/audience/scope/expiry 검사를 수행한다.
