...
- `world-tool`은 선택된 world root 밖 파일을 기본적으로 읽거나 쓰지 않는다. 예외는 두 가지다. 첫째, configured registry/config resolution과 `registry add/remove/default`를 위한 `--registry`, `WORLD_TOOL_REGISTRY`, `~/.opencrabs/worlds.yaml`, `~/.config/world-tool/worlds.yaml`의 명시적 읽기/쓰기이며, 이 경로들은 normalize 후 symlink 검사에 통과한 explicit configured path에만 접근하고 일반 home/config 탐색으로 확장하지 않는다. 이 예외는 world content, staging, run 경로를 선택된 world root 밖으로 허용하지 않는다. 둘째, `approval attest`가 `--auth-context-file`/`--auth-context-hash`로 검증하는 trusted auth context input 읽기이며, 이 파일도 쓰거나 run artifact로 복사하지 않는다. production auth context input은 먼저 configured wrapper-owned auth-context directory 아래의 normalized regular file이거나 동등하게 trusted wrapper request-file/FD mechanism인지 location validation을 통과해야 하며, parent traversal, symlink, selected world root, runtime-owned run directory, staged inbox path를 먼저 거부한 뒤에만 hash/parse/signature/MAC/trust-material 검증과 expected issuer/audience/scope/expiry 검증을 진행한다. local fixture mode는 test-only explicit opt-in exception이다. 이 input은 `world_id`, `allowed_actions`/scope, `issuer`, `audience`, `authenticated_actor`, `approval_channel`, and the exact `downstream_action`을 함께 묶는 wrapper provenance다. approval attestation은 `runs/inbox/*-approval-attestation.json`에 staged되는 별도 아티팩트이며, `world_create_approval_attestation`과 exact downstream action(`world_accept_draft` 또는 `world_force_accept_draft`)을 함께 bind해야 한다. auth context input의 exact downstream action이 없거나 `--downstream-action`과 mismatch면 `AUTH_CONTEXT_SCOPE_DENIED`다. attestation 생성만 허용하는 scope는 content mutation에 충분하지 않으며, staged attestation의 accept/force binding mismatch는 `APPROVAL_ATTESTATION_BINDING_MISMATCH`로 다룬다. 세부 계약은 `docs/commands.md`를 따른다.
...
- `approval attest`: `runs/inbox/**` write only for approval attestation artifacts; `--auth-context-file`/`--auth-context-hash` may read a trusted auth context input path outside the world root only after location validation against the configured wrapper-owned auth-context boundary or trusted wrapper request-file/FD mechanism, and only after rejecting parent traversal, symlink, selected world root, runtime-owned run directory, and staged inbox paths before hash/parse/signature/MAC/trust-material verification plus expected issuer/audience/scope/expiry checks. 승인 attestation은 `runs/inbox/*-approval-attestation.json`으로 staged되며 `world_create_approval_attestation`과 exact downstream action(`world_accept_draft` 또는 `world_force_accept_draft`)을 함께 bind해야 한다. attestation 생성만 허용하는 scope는 content mutation에 충분하지 않으며, 상세 필드 계약은 `docs/commands.md`를 따른다.
...
force accept는 가능하지만 reason만으로는 부족하다. `reason_file`/`reason_hash`, `approval_attestation_file`/`approval_attestation_hash`, `approver_id`, `approval_channel`, `authenticated_actor`를 함께 기록해야 하며, 이는 runs log와 accept result에 남아야 한다. `approval_channel` 예시는 `OpenCrabs-chat`을 사용하고, attestation, accept, audit 전반에서 byte-identical 값이어야 한다. `approval_channel`과 `authenticated_actor`는 `world_create_approval_attestation`이 확인한 trusted auth context input과 그로부터 staged된 approval attestation으로부터 와야 하며, production input은 먼저 configured wrapper-owned auth-context directory의 normalized regular file 또는 trusted wrapper request-file/FD mechanism인지 location validation을 통과한 뒤 parent traversal, symlink, selected world root, runtime-owned run directory, staged inbox path를 거부하고, 그 다음에 wrapper-signed 또는 MACed envelope와 configured trust material, expected issuer/audience/scope/expiry policy를 만족해야 한다. 그 input scope는 `world_create_approval_attestation`과 exact downstream action(`world_accept_draft` 또는 `world_force_accept_draft`)을 함께 authorize해야 하고, exact downstream-action binding이 없거나 mismatch면 `AUTH_CONTEXT_SCOPE_DENIED`로 실패해야 한다. attestation 생성만 허용하는 scope는 content mutation에 충분하지 않으며, staged attestation의 accept/force binding mismatch는 `APPROVAL_ATTESTATION_BINDING_MISMATCH`다. 모델 출력이나 staging file에서 오면 안 된다.
...
- actor
- timestamp
- force 여부, `reason_file`/`reason_hash`, redacted reason summary
...
- force 여부, `reason_file`/`reason_hash`, redacted reason summary, approval attestation/provenance를 runs log에 기록
...
- symlink resolution
- `--auth-context-file`은 staging file이 아니며, `approval attest`에서만 사용한다. production file은 먼저 configured wrapper-owned auth-context directory 아래의 normalized regular file이거나 trusted wrapper request-file/FD mechanism인지 location validation을 통과해야 한다. parent traversal, symlink, selected world root, runtime-owned run directory, staged inbox path는 hash/parse/signature/MAC/trust-material 검증 전에 거부하고, 그 다음 hash, parse, signature, MAC, trust-material 검증과 issuer/audience/scope/expiry 검사를 수행한다.
